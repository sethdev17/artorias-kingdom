document.addEventListener("DOMContentLoaded", () => {
  ...
  function clearFieldError(field) {
    const wrap =
      field.closest(".input-wrap") || field.closest(".textarea-wrap");
    if (!wrap) return;
    wrap.classList.remove("error");
    const help = wrap.querySelector(".field-error");
    if (help) help.remove();
  }
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (textarea && !textarea.value.trim()) {
      ...
      showFieldError(
        textarea,
        "Te rog introduce-ți mesajul pentru a-l trimite.",
      );
    }
      ...
      .catch((err) => {
        ...
        if (formMessage) {
          ...
          formMessage.textContent =
            "Eroare de conexiune. Serverul nu răspunde.";
        }
      });
  });
});
